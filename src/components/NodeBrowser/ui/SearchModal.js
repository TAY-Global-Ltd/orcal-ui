...
export const SearchButton = ({ onClick }) => {
  return (
    <button
      onClick={onClick}
      className="absolute top-4 right-4 bg-slate-900/90 backdrop-blur-sm p-2 rounded-lg border border-slate-800 shadow-xl hover:bg-slate-800 transition-colors"
      title="Search (Press /)"
    >
...
export const PathFinderButton = ({ onClick }) => {
  return (
    <button
      onClick={onClick}
      className="absolute top-4 right-14 bg-slate-900/90 backdrop-blur-sm p-2 rounded-lg border border-slate-800 shadow-xl hover:bg-slate-800 transition-colors"
...
export const SearchModal = ({
  ...
  return (
    <div
      className="absolute inset-0 bg-black/50 backdrop-blur-sm flex items-start justify-center pt-20 z-50"
      onClick={(e) => {
    ...
    >
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div className="flex items-center px-4 py-3 border-b border-slate-700">
          ...
          />
          <span className="text-xs text-slate-500 ml-2">ESC to close</span>
        </div>
          ...
          {searchResults.map((result, index) => (
            <button
              ...
              className={`w-full px-4 py-3 flex items-center gap-3 hover:bg-slate-800 transition-colors text-left ${
                index === selectedSearchIndex ? 'bg-slate-800' : ''
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-white font-medium truncate">{result.item.name}</span>
                  <span
